index_bind_type_1 <- function(sim_result,index,RP_name){ # data different from age
...
index_bind_type_2 <- function(sim_result,index,RP_name){ # data not different from age
  sim_result[[index]] %>%
...
index_bind_type_3 <- function(sim_result){ # data for fishing mortaltiy
...
index_bind_type_4 <- function(sim_result,index,RP_name){ # result for r,f,b
  sim_result[[index]] %>%
    apply(1, function(x) quantile(x, prob = c(0.05, 0.5, 0.95))) %>%
    t %>% as_tibble() %>%
    set_names(c("val_10", "val_50", "val_90")) %>%
...
scenario_and_management <- function(parameters,
  ...
  F_2_Bmsy_times <- function(Bmsy_times){ # function to calculate F for 1.5Bmsy、Bmsy、0.5Bmsy
    ny <- 100 # year for equivalent
  ...
  # the stock biomass at the statr of simulation is different for scenario
  naa <- caa <- wcaa <- faa <- baa <- ssb <- array(0,dim = c(na,130,sim))
  SBt <- iaa <- iaa_obs <- Catch <- matrix(0,130,sim)

  # custom=NULL: default parameters
  if(is.null(custom)){
  ...
  if(scenario_organization == "ICES"){
        ny_0.5Fmsy <- 75 # period with constant fishing mortality
        ny_history <- 25 # period with different fishing mortality for each fishing history
      ...
      run_hcr <- function(rule){
        if(rule == "chr_rule"){
          # reference all data-collective period for calculating f>=1
      ...
      if(is.null(custom)){
        ...
        if(is.null(GA)){
          ...
          #custom=1,GA=1:optimising parameters
        }else{
          fitness <- RC_long
          if(RSB_long >= 1){
            fitness <- fitness+5
...
# function for optimization with genetic argorithm
GA_result <- function(parameters,scenario_organization,scenario,start,end,rule){
  custom_monitor <- function(obj){
    ...
    # stop optimisation when fitness value exceed 10
  ...
  type2_config <- list(para_numb = 5,
    ...
    # funtion for implementing GA optimisation
  ...
  # performance and parameters of optimised
...
stock_data_func <- function(stock_name,ID,h_value=0.75){ # function for calculating biological parameters and reference points from stocks.csv
