Fig3_func <- function(MSE_output,parameters){
  ...
  make_plot <- function(data,xintercept,x_breaks,title_text){
    ...
    C_plot <- ggplot(filter(data,index == "C/MSY"),
                     aes(x = year,colour = method,linetype = method)) +
      geom_line(data = filter(data,index == "C/MSY"),aes(x = year,y = RP),
                colour = "black",linewidth = 2,alpha = 0.6,inherit.aes = FALSE) +
      geom_line(aes(y = mean),linewidth = 1) +
      geom_vline(xintercept = xintercept,lty = "31",col = "black") +
      labs(x = NULL,y = NULL) +
      scale_x_continuous(expand = expansion(mult = c(0,0.1)),breaks = x_breaks,
                         limits = c(head(x_breaks,1),tail(x_breaks,1))) +
      scale_y_continuous(expand = expansion(mult = c(0,0.1)),breaks = y_breaks_mean_C,
                         limits = c(0,tail(y_breaks_mean_C,1))) +
      scale_colour_manual(values = c("rfb rule" = "red",
                                     "rfb + c\u0304" = "orange",
                                     "type2 rule" = "blue",
                                     "type2 + f" = "cyan3",
                                     "chr rule" = "green"),
                          name = NULL) +
      scale_linetype_manual(values = c("rfb rule"   = "solid",
                                       "type2 rule" = "solid",
                                       "chr rule"   = "solid",
                                       "rfb + c\u0304" = "dashed",
                                       "type2 + f"    = "dashed"),
                            guide = "none") +
      guides(colour = guide_legend(reverse = TRUE)) + theme_classic() +
    ...
    SSB_plot <- ggplot(filter(data,index == "SSB/SBmsy"),
                       aes(x = year,colour = method,linetype = method)) +
      geom_line(data = filter(data,index == "SSB/SBmsy"),aes(x = year,y = RP),
                colour = "black",linewidth = 2,alpha = 0.6,inherit.aes = FALSE) +
      geom_line(aes(y = mean),linewidth = 1) +
      geom_vline(xintercept = xintercept,lty = "31",col = "black") +
      labs(x = "year",y = NULL) +
      scale_x_continuous(expand = expansion(mult = c(0,0.1)),breaks = x_breaks,
                         limits = c(head(x_breaks,1),tail(x_breaks,1))) +
      scale_y_continuous(expand = expansion(mult = c(0,0.1)),breaks = y_breaks_mean_SSB,
                         limits = c(0,tail(y_breaks_mean_SSB,1))) +
      scale_colour_manual(values = c("rfb rule" = "red",
                                     "rfb + c\u0304" = "orange",
                                     "type2 rule" = "blue",
                                     "type2 + f" = "cyan3",
    ...
    p <- (C_plot/SSB_plot) + plot_layout(heights = c(1,1))
    return(p)
  }
  
  # 各プロットの生成（必要な部分だけ指定）
  p1 <- make_plot(result_oneway[result_oneway$year >= 76,],100,c(76,100,130),"scenario (i)") + theme(plot.margin = margin(0,6,0,0))
  p2 <- make_plot(result_rollercoaster[result_rollercoaster$year >= 76,],100,c(76,100,130),"scenario (ii)") + theme(plot.margin = margin(0,6,0,6))
  p3 <- make_plot(result_confusion[result_confusion$year >= 76,],100,c(76,100,130),"scenario (iii)") + theme(plot.margin = margin(0,0,0,6))
  ...
  label_col <- make_label_panel("C/MSY")/make_label_panel("SSB/SBmsy") +
...
ggsave(paste0("./Figs_",parameters$fish,"/FIG3_mean_",parameters$fish,".jpg"),width = 340,height = 170,units = "mm",dpi = 300)
...
make_ribbon_plot <- function(data,xintercept,x_breaks,title_text){
  ...
  C_plot <- ggplot(filter(data,index == "C/MSY"),
                   aes(x = year,colour = method,linetype = method,group = method)) +
    geom_line(data = filter(data,index == "C/MSY"),aes(x = year,y = RP),colour = "black",
              linewidth = 2,alpha = 0.6,inherit.aes = FALSE) +
    geom_ribbon(aes(ymin = val_10,ymax = val_90,fill = method),alpha = 0.3,show.legend = FALSE) +
    ...
    scale_y_continuous(expand = expansion(mult = c(0,0.1)),
                       breaks = y_breaks_ribbon_C,
                       limits = c(0,tail(y_breaks_ribbon_C, 1))) +
  ...
  F_plot <- ggplot(filter(data,index == "F/Fmsy"),
                   aes(x = year,colour = method,linetype = method,group = method)) +
    geom_line(data = filter(data,index == "F/Fmsy"),aes(x = year,y = RP),colour = "black",
              linewidth = 2,alpha = 0.6,inherit.aes = FALSE) +
    geom_ribbon(aes(ymin = val_10,ymax = val_90,fill = method),alpha = 0.3,show.legend = FALSE) +
    geom_line(aes(y = val_10),linewidth = 0.5) +
    geom_line(aes(y = mean),linewidth = 1)   +
    ...
    scale_x_continuous(expand = expansion(mult = c(0,0.1)),
                       ...
                       limits = c(head(x_breaks,1),tail(x_breaks,1))) +
    scale_y_continuous(expand = expansion(mult = c(0,0.1)),
                       breaks = y_breaks_ribbon_F,
                       limits = c(0,tail(y_breaks_ribbon_F, 1))) +
    scale_colour_manual(values = method_cols,name = NULL) +
    scale_linetype_manual(values = method_ltys,name = NULL) +
    scale_fill_manual(values = method_cols,guide = "none") +
    guides(colour = guide_legend(reverse = TRUE),linetype = guide_legend(reverse = TRUE)) +
  ...
  SSB_plot <- ggplot(filter(data,index == "SSB/SBmsy"),
    ...
    scale_x_continuous(expand = expansion(mult = c(0,0.1)),
                       ...
                       limits = c(head(x_breaks,1),tail(x_breaks,1))) +
    scale_y_continuous(expand = expansion(mult = c(0,0.1)),
                       breaks = y_breaks_ribbon_SSB,
                       limits = c(0,tail(y_breaks_ribbon_SSB,1))) +
    scale_colour_manual(values = method_cols,name = NULL) +
    scale_linetype_manual(values = method_ltys,name = NULL) +
    scale_fill_manual(values = method_cols,guide = "none") +
    guides(colour = guide_legend(reverse = TRUE),linetype = guide_legend(reverse = TRUE)) +
...
p1 <- make_ribbon_plot(result_oneway[result_oneway$year >= 76,],100,c(76,100,130),"scenario (i)") + theme(plot.margin = margin(0,6,0,0))
p2 <- make_ribbon_plot(result_rollercoaster[result_rollercoaster$year >= 76,],100,c(76,100,130),"scenario (ii)") + theme(plot.margin = margin(0,6,0,6))
p3 <- make_ribbon_plot(result_confusion[result_confusion$year >= 76,],100,c(76,100,130),"scenario (iii)") + theme(plot.margin = margin(0,0,0,6))
...
(label_col | p1 | p2 | p3) + plot_layout(widths = c(0.2,1,1,1),guides = "collect") +
  plot_annotation(title = title_name,
                  theme = theme(plot.title.position = "plot",
                                ...
                                legend.key.width = unit(20,"pt"),
                                legend.key.height = unit(20,"pt")))
ggsave(paste0("./Figs_",parameters$fish,"/FIG3_ribbon_",parameters$fish,".jpg"),width = 340,height = 170,units = "mm",dpi = 300)
}