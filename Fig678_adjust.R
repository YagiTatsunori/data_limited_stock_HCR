...
# --------- データ読み込み（元の load_data 関数を継承） ----------
load_data <- function(stock_name,method){
  file_path <- paste0("C:/Users/00008252/OneDrive - 国立研究開発法人 水産研究・教育機構/デスクトップ/論文提出用/data_limited_HCR_BMSY/",
                      stock_name,"/GA/generation_populations_",method,"_",stock_name,".csv")
...
# --------- 4魚種を統合し、scenario内で stock を並べて箱ひげ図を作成する関数 ----------
...
param_distribution_allstocks <- function(parameter, default, method, fig_id, title_name){
  ...
  # ---- 参照線 ----
  ref_lines <- if (parameter == "m") c(default, 0.9, 0.5) else default

  # ★ 表示ラベルの整形（Blimit → PL だけ置換）
  ...
  save_path <- paste0("C:/Users/00008252/OneDrive - 国立研究開発法人 水産研究・教育機構/デスクトップ/論文提出用/data_limited_HCR_BMSY/",
                      "Figs_allstocks/Figs", fig_id, "_", method, "_", parameter, "_allstocks.jpg")
