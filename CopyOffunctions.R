...
scenario_and_management <- function(parameters,
  ...
  if(is.null(custom)){
    ...
    if(is.null(GA)){
      ...
      # ここからYagi and Yamakawa (2022)ルール
      if(rule == "YY_rule"){
        for(t in (ny_before+1):ny){
          naa[1,t,] <- (alpha*SBt[t-1,]/(beta+SBt[t-1,]))*exp(epsiron_r[(t-ny_before),]-0.5*sd_r^2) # Beverton-Holt type reproductive function
          naa[2:(na-1),t,] <- naa[1:(na-2),t-1,]*exp(-faa[1:(na-2),t-1,]-M[1:(na-2)])
          ...
          for(k in 1:sim){
            data_input <- data.frame(year = (ny_before-ny_reference):(t-2), cpue = iaa_obs[(ny_before-ny_reference):(t-2),k], catch = Catch[(ny_before-ny_reference):(t-2),k])
            cpue <- data_input[,2]
            D <- pnorm(scale(cpue),0,1)[t-ny_before]
            coef <- (D^alpha_YY-Btarget^alpha_YY)*beta_YY+1
            ABC[t,k] <- coef*Catch[t-2,k] # 2年前までのデータを使用
          }
          Catch[t,] <- ABC[t,]
...
GA_result <- function(parameters, scenario_organization, scenario, start, end, rule){
  ...
  if(rule == "rfb_rule" | rule == "ICES_average"){
    ...
    initial_population <- matrix(runif(30*para_numb, min = 0, max = 1), nrow = 30, ncol = para_numb)
    specified_individuals <- matrix(rep(c(m_default, 0.4, 0.75), 20), nrow = 20, byrow = TRUE)
    suggestions <- rbind(specified_individuals, initial_population)
    suggestions <- as.data.frame(suggestions)
    GA_HCR <- function(parameters, scenario_organization, scenario, start, end, rule){
      ga(type = "real-valued",
         ...
         popSize = popsize, maxiter = maxiter, run = run, parallel = TRUE, keepBest = TRUE, monitor = customMonitor, seed = 1234)
    }}
  ...
  if(rule == "chr_rule"){
    ...
    initial_population <- matrix(runif(30*para_numb, min = 0, max = 1), nrow = 30, ncol = para_numb)
    specified_individuals <- matrix(rep(m_default, 20), nrow = 20, byrow = TRUE)
    suggestions <- rbind(specified_individuals, initial_population)
    suggestions <- as.data.frame(suggestions)
    GA_HCR <- function(parameters, scenario_organization, scenario, start, end, rule){
      ga(type = "real-valued",
         fitness =  function(x) scenario_and_management(parameters,
                                                        ...
                                                        rule,
                                                        m = x[1]),
         lower = c(0), upper = c(1), suggestions = suggestions,
         popSize = popsize, maxiter = maxiter, run = run, parallel = TRUE, keepBest = TRUE, monitor = customMonitor, seed = 1234)
    }}
  ...
  if(rule == "YY_rule"){
    para_numb <- 3
    initial_population <- matrix(runif(30*para_numb, min = 0, max = 1), nrow = 30, ncol = para_numb)
    specified_individuals <- matrix(rep(c(0.8, 1, 1), 20), nrow = 20, byrow = TRUE)
    suggestions <- rbind(specified_individuals, initial_population)
    suggestions <- as.data.frame(suggestions)
    GA_HCR <- function(parameters, scenario_organization, scenario, start, end, rule){
      ga(type = "real-valued",
...
func <- function(parameters = stock_parameters(fish_data),
  ...
  if(is.null(GA)){
    ...
    # genetic algorithm for optimizing the parameters of chr rule
    GA_result_YY <- GA_result(parameters, scenario_organization, scenario, start, end, rule = "YY_rule")

    MSE_output <- tibble(GA_result_type2_rule = GA_result_type2_rule,
                         GA_result_rfb_rule = GA_result_rfb_rule,
                         GA_result_type2_rule_length = GA_result_type2_rule_length,
                         GA_result_ICES_average = GA_result_ICES_average,
                         GA_result_chr = GA_result_chr,
                         GA_result_YY = GA_result_YY)
    return(MSE_output)
  }}
