FigS1_func <- function(parameters){
  ...
  figure1_func <- function(F,SB,sub_name,scenario){
    ...
    ggsave(paste0("./Figs_",parameters$fish,"/",scenario,"_faa.jpg"),
    ...
    ggplot(data,aes(x = year, y = SB.1)) +
      ...
      scale_x_continuous(expand = expansion(mult = c(0,0.1)),breaks = c(76,80,90,100),limits = c(76,100)) +
      scale_y_continuous(expand = expansion(mult = c(0,0.1)),breaks = c(0,500),limits = c(0,500)) +
      geom_line(aes(y = RP),col = "black",linewidth = 3,linetype = "dashed") +
      labs(title = sub_name[2],x = "year",y = "Spawning biomass (SSB)") +
      theme_bw() + theme_classic() +
      theme(plot.title.position = "plot",
            plot.title = element_text(hjust = 0.1,vjust = 0,size = 30,color = "black"),
            axis.text = element_text(size = 30,color = "black"),
            axis.title = element_text(size = 30,color = "black"),
            axis.line = element_line(colour = "black",linewidth = 1,lineend = "square"),
            axis.ticks.length = unit(0.3,"cm"),
            axis.ticks = element_line(linewidth = 1))
    ggsave(paste0("./Figs_",parameters$fish,"/",scenario,"_ssb.jpg"),
           width = 170, height = 170, units = "mm", dpi = 300)
  }
  
  figure1_func(F = F_one,SB = SB_one,sub_name = c("Scenario i",""),scenario = "oneway")
  figure1_func(F = F_roller,SB = SB_roller,sub_name = c("Scenario ii",""),scenario = "rollercoaster")
  figure1_func(F = F_conf,SB = SB_conf,sub_name = c("Scenario iii",""),scenario = "confusion")
}