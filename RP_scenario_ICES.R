...
ver_stk <- 0.0002 # initial stock biomass for each age
a <- 0.0076; b <- 3.069 # parameters in allometry 
L_inf <- 85.6; t0 <- -0.1; k_von <- 0.19 # von Bertalanffy growth parameters 
L50 <- 47.1; a50 <- 4.105405 # the length or age at 50% maturity
...
stock(stock_data) <- computeStock(stock_data)       # derive stock amount every year by multiply individual weight by stock number
...
ICES_func <- function(){
  ...
  for(k in 1:sim){
    ...
    for(t in 1:100){
      ...
      for(i in 1:na){
        age_length <- floor(seq(laa[i]-2, laa[i]+2, by=1)) # range of length for each age
        probs <- dnorm(age_length, laa[i], 0.2) # probability of length for each age
        probs <- probs/(sum(probs)) # normalized probability
        number_length <- caa[i,t,k]*probs # derive the number of each length from all catch number
        age_data <- data.frame(year = t, length = age_length, numbers = number_length)
        frequency_length <- rbind(frequency_length,age_data) # plus the number of each length for every year
      }
