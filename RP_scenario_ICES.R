...
names(RP_ICES) <- c("Fmsy", "MSY", "SBmsy", "SB0")
################################################################################
...
simulation_result_U <- rbind(
  result_ICES$U %>%
    ...
    t %>% as_tibble() %>%
    set_names(c("val_10", "val_50", "val_90")) %>%
    mutate(mean = result_ICES$U %>% apply(1, mean),
           method = "ICES", year = 1:(ny_msy+ny_man),
           No1 = result_ICES$U[,1],
           No2 = result_ICES$U[,round(sim/2)],
           No3 = result_ICES$U[,sim])
)
...
ggplot(simulation_result_BAA[simulation_result_BAA$year >= 50,],aes(year,colour = method)) +
    geom_ribbon(aes(ymin = val_10, ymax = val_90, fill = method), alpha = 0.3) +
    geom_line(aes(y = val_10), size = 0.5) +
    geom_line(aes(y = mean), size = 1) +
    geom_line(aes(y = val_90), size = 0.5) +
...
ggplot(simulation_result_U[simulation_result_U$year >= 50,],aes(year,colour = method)) +
  geom_ribbon(aes(ymin = val_10, ymax = val_90, fill = method), alpha = 0.3) +
  geom_line(aes(y = val_10), size = 0.5) +
  geom_line(aes(y = mean), size = 1) +
  geom_line(aes(y = val_90), size = 0.5) +
